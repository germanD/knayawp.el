...
;;;; send-to-terminal (#162)

(ert-deftest knayawp-test-send-to-terminal-no-region ()
  "`knayawp-send-to-terminal' signals user-error when no region is active."
  (cl-letf (((symbol-function 'use-region-p) (lambda () nil)))
    (should-error (knayawp-send-to-terminal) :type 'user-error)))

(ert-deftest knayawp-test-send-to-terminal-saves-region ()
  "`knayawp-send-to-terminal' calls kill-ring-save with the region bounds."
  (let ((saved-args nil)
        (knayawp-terminal-backend 'vterm))
    (cl-letf (((symbol-function 'use-region-p) (lambda () t))
              ((symbol-function 'region-beginning) (lambda () 10))
              ((symbol-function 'region-end) (lambda () 20))
              ((symbol-function 'kill-ring-save)
               (lambda (beg end &optional _r) (setq saved-args (list beg end))))
              ((symbol-function 'knayawp--active-terminal-window)
               (lambda () 'fake-win))
              ((symbol-function 'select-window) #'ignore)
              ((symbol-function 'knayawp--make-terminal-yank-vterm) #'ignore))
      (knayawp-send-to-terminal)
      (should (equal saved-args '(10 20))))))

(ert-deftest knayawp-test-send-to-terminal-dispatches-vterm ()
  "`knayawp-send-to-terminal' calls the vterm yank helper."
  (let ((knayawp-terminal-backend 'vterm)
        (dispatched-to nil))
    (cl-letf (((symbol-function 'use-region-p) (lambda () t))
              ((symbol-function 'region-beginning) (lambda () (point-min)))
              ((symbol-function 'region-end) (lambda () (point-max)))
              ((symbol-function 'kill-ring-save) #'ignore)
              ((symbol-function 'knayawp--active-terminal-window)
               (lambda () 'fake-win))
              ((symbol-function 'select-window) #'ignore)
              ((symbol-function 'knayawp--make-terminal-yank-vterm)
...
(ert-deftest knayawp-test-send-to-terminal-dispatches-eat ()
  "`knayawp-send-to-terminal' calls the eat yank helper."
  (let ((knayawp-terminal-backend 'eat)
        (dispatched-to nil))
    (cl-letf (((symbol-function 'use-region-p) (lambda () t))
              ((symbol-function 'region-beginning) (lambda () (point-min)))
...
(ert-deftest knayawp-test-send-to-terminal-unknown-backend ()
  "`knayawp-send-to-terminal' signals user-error for unknown backend."
  (let ((knayawp-terminal-backend 'nonexistent))
    (cl-letf (((symbol-function 'use-region-p) (lambda () t))
              ((symbol-function 'region-beginning) (lambda () (point-min)))
              ((symbol-function 'region-end) (lambda () (point-max)))
